...
const VisitsTable: React.FC<VisitsTableProps> = ({ visits, onEdit, onDelete }) => {
  ...
  return (
    <div>
      ...
      <table id="visits-table" className="min-w-full divide-y divide-gray-300 border-collapse [&_th]:border [&_th]:border-gray-300 [&_td]:border [&_td]:border-gray-300">
        ...
        <tbody className="divide-y divide-gray-200 bg-[#ece8e3]">
          {visibleVisits.map((visit, index) => (
            <tr key={visit.id}>
              ...
              <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{formatDateDisplay(visit.scheduledDate)}</td>
              <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{visit.scheduledTime || 'N/A'}</td>
              <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{formatDateDisplay(visit.visitDate)}</td>
