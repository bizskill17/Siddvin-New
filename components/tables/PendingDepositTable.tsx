    ...
    return (
        <div>
            <div className="mb-4 flex flex-wrap items-center justify-end gap-2">
                ...
                <Button size="sm" variant="secondary" onClick={() => downloadTableAsPdf('pending-deposit-table', 'Pending Deposits')}>
                    Download PDF
                </Button>
            </div>
                ...
                <table id="pending-deposit-table" className="min-w-full divide-y divide-gray-300 border-collapse [&_th]:border [&_th]:border-black [&_td]:border [&_td]:border-black">
                    <thead className="bg-orange-700 text-white">
                        <tr>
                            ...
                            <th scope="col" className="px-3 py-3.5 text-center text-sm font-semibold text-white border-b border-black">
                                Update
                            </th>
                        </tr>
                    </thead>
                        ...
                        {visibleRows.length === 0 ? (
                            ...
                            visibleRows.map((row) => (
                                <tr key={row.proposal.id}>
                                    ...
                                    <td className="whitespace-nowrap px-3 py-4 text-sm text-black">{row.propertyAddress}</td>
                                    <td className="whitespace-nowrap px-3 py-4 text-sm text-black">{row.brandName}</td>
                                    <td className="whitespace-nowrap px-3 py-4 text-sm text-black">{row.totalDeposit.toLocaleString()}</td>
                                    <td className="whitespace-nowrap px-3 py-4 text-sm text-black">{row.depositReceived.toLocaleString()}</td>
                                    <td className="whitespace-nowrap px-3 py-4 text-sm text-black">{row.nextDepositName}</td>
                                    <td className="whitespace-nowrap px-3 py-4 text-sm text-black">{row.nextDepositAmount.toLocaleString()}</td>
